...
const PathMeshEditor = {
  ...
  create_controls() {
    this.controls = this.geometry.points.map((point) => ({ point }));
  },
...
export default {
  ...
  _check_intersects(annotation, mousePosition) {
    ...
    } if (annotation.geometry.type === 'polygon' || annotation.geometry.type === 'brush') {
  ...
  init(Viewer, callback) {
    ...
    this.viewer.addHandler('update-viewport', () => {
      ...
      if (viewportMin.x !== this._viewportLocation.min.x
        ...
        // The viewport was changed, an update is needed
    ...
    new OpenSeadragon.MouseTracker({
      ...
      moveHandler: (e) => {
        ...
        } else if (this.currently_editing.mouseEvent('move', e)) return;
