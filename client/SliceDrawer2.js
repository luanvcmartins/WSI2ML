...
class StateRestorer {
  constructor(instance, annotation) {
    ...
    console.log(this.instance.callback);
    if (this.instance.callback != null) {
...
class Annotation {
  constructor(drawer, annotation) {
    ...
    this.flatCoords = this.geometry.points.map(element => [element.x, element.y])
      .flat();
    const indices = earcut(this.flatCoords);

    this.vertices = new Float32Array(indices.length * 2);

    for (let i = 0; i < indices.length; i++) {
      this.vertices[i * 2] = this.flatCoords[indices[i] * 2];
      this.vertices[i * 2 + 1] = this.flatCoords[indices[i] * 2 + 1];
    }
    console.log('Created vertices: ', this.vertices);
  ...
  shouldBeVisible(userViewport) {
    const onScreen = this.imageLocation.top <= userViewport.bottom && this.imageLocation.bottom >= userViewport.top && this.imageLocation.left <= userViewport.right && this.imageLocation.right >= userViewport.left;
    const { filtering } = this.drawer;
    const isFilteredOut = !(this.state === 'importing' || this.state === 'overlay') && filtering[this.label.name] === true;
    return onScreen && !isFilteredOut;
  }
...
class PolygonAnnotation extends Annotation {
  ...
  draw() {
    console.log('Called draw on PolygonAnnotation: ', this.vertices, hexToRgb(this.label.color));
    this.drawer.gl.bindBuffer(this.drawer.gl.ARRAY_BUFFER, this.drawer.buffer);
    this.drawer.gl.bufferData(this.drawer.gl.ARRAY_BUFFER, this.vertices, this.drawer.gl.STREAM_DRAW);

    this.drawer.gl.uniform4fv(this.drawer.uniforms.u_color, hexToRgb(this.label.color));

    this.drawer.gl.vertexAttribPointer(
      this.drawer.attribs.a_position, 2,
      this.drawer.gl.FLOAT, false,
      0, 0);
    this.drawer.gl.enableVertexAttribArray(this.drawer.attribs.a_position);

    // this.drawer.gl.drawArrays(this.drawer.gl.TRIANGLES, 0, this.vertices.length / 2);

    // border
    this.drawer.gl.bindBuffer(this.drawer.gl.ARRAY_BUFFER, this.drawer.borderBuffer);
    this.drawer.gl.vertexAttribPointer(this.drawer.attribs.a_position, 2, this.drawer.gl.FLOAT, false, 0, 0);
    this.drawer.gl.bufferData(this.drawer.gl.ARRAY_BUFFER, this.vertices, this.drawer.gl.STREAM_DRAW);

    this.drawer.gl.uniform4fv(this.drawer.uniforms.u_color, hexToRgb(this.label.color));
    this.drawer.gl.lineWidth(5);
    this.drawer.gl.drawArrays(this.drawer.gl.LINE_LOOP, 0, this.flatCoords.length / 2);
  }
}
...
function hexToRgb(hex) {
  ...
  // Expande a string hexadecimal para 6 caracteres, se necessário
  if (hex.length === 3) {
    hex = hex[0] + hex[0] + hex[1] + hex[1] + hex[2] + hex[2];
  }

  // Converte os caracteres hexadecimais para valores decimais
...
class AnnotationDrawer {
  constructor(Viewer, callback) {
    ...
    this.gl.clearColor(0.0, 0.0, 0.0, 0.0); // RGBA
    ...
    this.style = [{ fillOpacity: 0.1 }, { fillOpacity: 0.6 }];
    this.currentViewport = {
      top: 0,
      bottom: 0,
      left: 0,
    ...
    this.viewer.addHandler('update-viewport', _.throttle(this.updateViewport.bind(this), 10));
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (!this.isPanningEnabled) {
        if (e.key === ' ' || e.key === 'Space') { // SPACE key
  ...
  editAnnotation(annotationId) {
    ...
    const annotation = this.annotationMap[annotationId];
  ...
  loadAnnotations(annotations, layer = 0) {
    ...
    this.annotationSet[layer] = annotations.map((annotation) => {
      ...
      this.annotationMap[annotation._id] = annotationInstance;
  ...
  createCameraMatrix() {
    const width = (this.currentViewport.right - this.currentViewport.left);
    const height = (this.currentViewport.bottom - this.currentViewport.top); // flip Y
    const x = this.currentViewport.left;
    const y = this.currentViewport.top;
    ...
    // Shift origin to center and flip Y
    const txClip = -1;
    const tyClip = 1;
  ...
  update() {
    ...
    console.log('Camera matrix: ', matrix, 'from', this.currentViewport);
    this.gl.clear(this.gl.COLOR_BUFFER_BIT);
    this.gl.uniformMatrix3fv(this.uniforms.u_matrix, false, matrix);
