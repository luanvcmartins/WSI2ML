<template>
  <v-container>
    <v-card variant="text" v-for="pe in projectExports" :key="pe._id" :title="pe.name" :subtitle="pe.description">
      <template v-slot:append>
        <v-btn variant="outlined" color="primary" prepend-icon="mdi-plus" @click="loadVersionCreation(pe._id)" rounded size="large">
          Create new dataset version
        </v-btn>
      </template>
      <v-card-text>
        <div class="d-flex flex-wrap justify-center mt-4" v-if="pe.versions.length > 0">
          <v-card variant="outlined" class="ma-2" v-for="version in pe.versions" :title="version.title"
          ...
          <v-form fast-fail v-model="isFormValid">
            <v-row no-gutters>
              <v-col cols="12">
                <v-text-field
                    label="Title"
                    v-model="newDatasetRequest.title"
                    :rules="fieldRequired"
                    outlined
                    required
                ></v-text-field>
            ...
            <v-row>
              <v-col cols="12">
                <v-textarea
                    label="Description"
                    v-model="newDatasetRequest.description"
                    :rules="fieldRequired"
                    outlined
                    required
                ></v-textarea>
            ...
            <v-row>
              <v-col cols="6">
                <v-select
                    label="Include all annotations from"
                    :items="newProjectExport.users"
                    v-model="newDatasetRequest.users"
                    multiple
                    item-value="_id"
                    item-title="name"
                    outlined
                    color="green"
                    variant="outlined"
                    hint="All annotations from the selected users will be exported"
                    persistent-hint
                ></v-select>
              </v-col>
              <v-col cols="6">
                <v-select
                    label="Not include annotations flagged by"
                    :items="newProjectExport.users"
                    multiple
                    v-model="newDatasetRequest.not_flagged_by"
                    outlined
                    item-value="_id"
                    item-title="name"
                    color="red"
                    variant="outlined"
                    hint="Annotation flagged by the selected users will not be exported"
                    persistent-hint
                ></v-select>
              </v-col>
            </v-row>
    ...
    <v-dialog v-model="constructionProgress.dialog" persistent max-width="600px">
      <v-card title="Creating dataset version" subtitle="Please wait while the dataset version is being created">
        <v-card-text class="text-center">
          <v-progress-circular
              :model-value="(constructionProgress.info.step / 5)*100"
              color="primary"
              size="80"
              width="6"
              :indeterminate="constructionProgress.info.step === 3"
          >
            {{ constructionProgress.info.step }} / 5
          </v-progress-circular>
          <p class="mt-3">{{ constructionProgress.info.msg }}</p>
          <v-progress-linear
              :model-value="constructionProgress.info.progress * 100"
              v-if="constructionProgress.info.step === 3"
              height="10"
              color="primary"/>
          ...
          <v-card variant="outlined" class="mt-3" v-if="constructionProgress.info.step >= 3"
            ...
            <v-card-text>
              <span v-for="[label, count] in Object.entries(constructionProgress.info.annotation_count)"
                    :key="label"> {{ label }}
...
function loadVersionCreation(projectId) {
  $axios.get(`/export/${projectId}/prepare_version`)
      .then((res) => {
...
function loadExports() {
  $axios.get('/export/list')
      .then((res) => {
        projectExports.value = res.data;
      })
      .catch((err) => {
        Swal.fire({
          icon: 'error',
