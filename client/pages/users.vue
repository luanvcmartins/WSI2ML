<template>
  <v-container>
    <v-card title="Users" density="compact" subtitle="Manage users">
      <template v-slot:append>
        <v-btn variant="flat" prepend-icon="mdi-account-plus" @click="newUser">New user</v-btn>
      </template>

      <v-data-table :headers="table" density="compact" :items="users">
        ...
        <template v-slot:item.actions="{ item }">
          ...
          <v-btn :disabled="item.email === 'admin'" :icon="item.enabled ? 'mdi-delete' : 'mdi-check'" size="small"
            @click="switchAccess(item._id, item.enabled)" variant="plain" />
        </template>
      </v-data-table>
    </v-card>
    <v-dialog v-model="dialog" max-width="500px" :close-on-back="false" :close-on-content-click="false">
      <v-card>
        <v-card-title>User editor</v-card-title>
        <v-card-text>
          <v-form id="user-form" v-model="isValid" fast-fail>
            <v-text-field :rules="fieldRequired" label="Name" v-model="user.name"></v-text-field>
            <v-text-field :rules="fieldRequired" label="Username" v-model="user.email"></v-text-field>
            <v-text-field :rules="fieldRequired" label="Password" type="password"
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn @click="dialog = false" :disabled="isLoading" :loading="isLoading">
            Cancel
          </v-btn>
          <v-btn :text="'_id' in user ? 'Update user' : 'Create user'" @click="submit" :disabled="isLoading || !isValid"
            :loading="isLoading" />
        </v-card-actions>
      </v-card>
    </v-dialog>
