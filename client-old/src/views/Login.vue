    ...
    <v-row class="fill-height">
      ...
      <v-col class="fill-height text-xs-center" style="position: relative;" md="6" lg="4">
        <v-card class="centered-card">
          ...
          <v-card-text>
            <!--            <v-form>-->
            <v-text-field v-model="login_info.email" label="E-mail"></v-text-field>
            <v-text-field
          ...
          <v-card-actions>
            <v-btn @click="createAdmin">First access?</v-btn>
            <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    createAdmin() {
      this.$post('/user/create_admin').then((resp) => {
        this.$toast.info('Request send.');
      });
    },
    login() {
